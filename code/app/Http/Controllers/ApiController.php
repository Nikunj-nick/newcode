...
class ApiController extends Controller
{
        ...
        if (!$validator->fails()) {
            ...
            $current_package = UserPurchasedPackage::where('modal_id', $current_user)
                ->with(['package' => function ($q) {
                    $q->select('id', 'property_limit')->where('property_limit', '>', 0)->orWhere('property_limit', null);
                }])
            ...
            if (!($current_package)) {
                ...
                $actualpackage = Package::find($current_package->package_id);


                $package = UserPurchasedPackage::where('modal_id', $current_user)->where('package_id', $actualpackage->id)->where('prop_status', 1)->first();
                ...
                if (($package->used_limit_for_property) < ($prop_count) || $prop_count == 0) {
                    ...
                    $title = $request->title;
                    $Saveproperty->slug_id = generateUniqueSlug($title, $title,1);
    ...
    public function store_advertisement(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $current_package = UserPurchasedPackage::where('modal_id', $current_user)
                ->with(['package' => function ($q) {
                    $q->select('id', 'advertisement_limit')->where('advertisement_limit', '>', 0)->orWhere('advertisement_limit', null);
                }])
                ->whereHas('package', function ($q) {
            ...
            if (!($current_package)) {
                ...
                $package = UserPurchasedPackage::where('modal_id', $current_user)->where('package_id', $actualpackage->id)->where('adv_status', 1)->first();
                $advertisement_count = 0;

                if (!$package->package) {

                    $response['error'] = false;
                    $response['message'] = 'Package not found for advertisement';
                    return response()->json($response);
                }
                $advertisement_count = $package->package->advertisement_limit;
                if (($package->used_limit_for_advertisement) < ($advertisement_count) || $advertisement_count == 0) {
                    ...
                    if ($package) {
                        ...
                        if ($request->type == 'Slider') {
                            ...
                            $proeprty = Property::find($request->property_id);
                            $slider->category_id = isset($request->category_id) ? $request->category_id : $proeprty->category_id;
                        ...
                        $package->used_limit_for_advertisement
                            =  $package->used_limit_for_advertisement + 1;
                        $package->save();
    ...
    public function get_package(Request $request)
    {
        ...
        $packages->transform(function ($item) use ($request) {
            if ($request->filled('current_user')) {
                ...
                $user_package = UserPurchasedPackage::where('modal_id', $request->current_user);
    ...
    public function post_project(Request $request)
    {
        ...
        $seo_settings = \system_setting('seo_settings');

        if (!$validator->fails()) {
            $payload = JWTAuth::getPayload($this->bearerToken($request));
            $current_user = ($payload['customer_id']);
            if (!(isset($request->id))) {
                $current_package = check_subscription($current_user, 'property_limit', 'prop_status');
                ...
                $project = Projects::where('added_by', $current_user)->find($request->id);
                if (!$project) {
                    $response['error'] = false;
            ...
            if ($request->id) {
                ...
                if ($project->title !== $request->title) {
                    ...
                    $project->title = $title;
                    $project->slug_id = generateUniqueSlug($title,null,4);
                } else {
                    $title = $request->title;
                    $project->title = $title;
                }
    ...
    public function getUserPersonalisedInterest(Request $request)
    {
        try {
            ...
            $userInterest = UserInterest::where('user_id', $current_user)->first();
    ...
    public function storeUserPersonalisedInterest(Request $request)
    {
        try{
            ...
            // Change the values
            $userInterest->user_id = $current_user;
    ...
    public function deleteUserPersonalisedInterest(Request $request)
    {
        try{
            ...
            // Get User Interest
            UserInterest::where('user_id', $current_user)->delete();
            DB::commit();
            $response = array(
                'error' => false,
    ...
    public function removeAllPackages(Request $request)
    {
        try{
            ...
            $customerData = Customer:: find($current_user);
